...
const PillVisualizer: React.FC = () => {
  ...
  // Corrected supported ratios for gemini-3-pro-image-preview
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {sizes.map(s => (
                  <button 
                    ...
                    className={`flex-1 py-2 rounded-xl text-xs font-black border transition-all ${
                      imageSize === s ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg' : 'bg-white text-slate-400 border-slate-200 hover:border-indigo-300'
                    }`}
                  >
                    {s}
                  </button>
                ...
                {ratios.map(r => (
                  <button 
                    ...
                    className={`px-2 py-2 rounded-xl text-[10px] font-black border transition-all ${
                      aspectRatio === r ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'bg-white text-slate-400 border-slate-200 hover:border-blue-300'
                    }`}
                  >
                    {r}
                  </button>
        ...
        <Card title="Visual Output">
          <div className="flex flex-col items-center justify-center min-h-[400px] bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-100">
            {image ? (
