...
const HealthProfileWizard: React.FC<Props> = ({ onComplete }) => {
  ...
  const handleComplete = () => {
    ...
    // Final check for required fields in the last step
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 py-8 px-4 animate-in fade-in duration-700">
      <div className="text-center space-y-2">
         <h1 className="text-4xl font-black text-slate-900 tracking-tight">Setup Your Profile</h1>
         <p className="text-slate-500 font-medium">To provide accurate safety warnings, we need your health details.</p>
      </div>
      ...
      <div className="flex justify-between items-center px-4 relative">
         {[1, 2, 3].map(i => (
           <div key={i} className={`z-10 w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all border-4 ${
             step >= i ? 'bg-blue-600 text-white border-blue-100' : 'bg-white text-slate-300 border-slate-100'
           }`}>
             {i}
           </div>
         ))}
      ...
      <Card>
        {step === 1 && (
          <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
            ...
            <div className="space-y-1.5">
               <label className="text-sm font-semibold text-slate-700 ml-1">Blood Type</label>
               <select className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
                 ...
                 {profile.allergies.map(a => (
                   <Badge key={a}>
                      ...
                      <button onClick={() => setProfile(p => ({...p, allergies: p.allergies.filter(x => x !== a)}))}>
                         <i className="fas fa-times ml-1 text-slate-400 hover:text-red-500"></i>
                      </button>
                   </Badge>
                 ))}
                 {profile.allergies.length === 0 && <p className="text-sm text-slate-400 italic">No drug allergies listed...</p>}
              </div>
            </div>
                 ...
                 {profile.medicalConditions.map(c => (
                   <Badge key={c} color="blue">
                      ...
                      <button onClick={() => setProfile(p => ({...p, medicalConditions: p.medicalConditions.filter(x => x !== c)}))}>
                         <i className="fas fa-times ml-1 text-slate-400 hover:text-blue-500"></i>
                      </button>
                   </Badge>
                 ))}
                 {profile.medicalConditions.length === 0 && <p className="text-sm text-slate-400 italic">No medical conditions listed...</p>}
              </div>
            </div>
